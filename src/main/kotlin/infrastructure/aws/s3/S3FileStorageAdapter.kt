...
class S3FileStorageAdapter(config: ApplicationConfig, private val s3ClientFactory: S3ClientFactory) : FileStoragePort {
    ...
    override suspend fun objectExists(key: String): Boolean = try {
        withContext(Dispatchers.IO) {
            s3ClientFactory.createClient(region).use { s3Client ->
                s3Client.headObject(
                    HeadObjectRequest {
    ...
    override suspend fun deleteObject(key: String): Boolean = try {
        withContext(Dispatchers.IO) {
            s3ClientFactory.createClient(region).use { s3Client ->
...
class ConfigBasedS3ClientFactory(config: ApplicationConfig) : S3ClientFactory {
