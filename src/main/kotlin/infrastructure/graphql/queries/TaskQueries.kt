...
@Single
class TaskQueries(private val taskDetailService: TaskDetailService) {
    ...
    suspend fun taskById(id: Int): TaskGQL? = taskDetailService.getTaskById(id)?.toGQL()

    suspend fun taskByName(name: String): TaskGQL? = taskDetailService.getTaskByName(name)?.toGQL()
}

fun Priority.toGQL(): PriorityGQL = when (this) {
...
fun PriorityGQL.toDomain(): Priority = when (this) {
...
fun domain.model.TaskDetail.toGQL(): TaskGQL = TaskGQL(
    id = id,
    name = name,
    description = description,
...
fun TaskFilterGQL.toDomain(): TaskFilter = TaskFilter(
...
fun TaskSortFieldGQL.toDomain(): TaskSortField = when (this) {
...
fun SortDirectionGQL.toDomain(): SortDirection = when (this) {
