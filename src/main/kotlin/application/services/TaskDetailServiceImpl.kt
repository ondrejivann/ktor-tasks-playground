...
class TaskDetailServiceImpl(
    ...
    override suspend fun getTaskDetailByPriority(priority: Priority): List<TaskDetail> =
        taskService.tasksByPriority(priority).map {
            mapToTaskDetail(it)
        }

    override suspend fun getTaskByName(name: String): TaskDetail? = taskService.taskByName(name)?.let {
        mapToTaskDetail(it)
    }

    override suspend fun getTaskById(id: Int): TaskDetail? = taskService.taskById(id)?.let {
        mapToTaskDetail(it)
    }
    ...
    override suspend fun addTask(command: CreateTaskCommand): CreateTaskCommandResponse {
        ...
        val taskAttachmentUploads =
            command.attachments.map { attachmentRequest ->
                val fileUploadInfo =
    ...
    override suspend fun updateTaskStatus(name: String, statusCode: String): Boolean =
        taskService.updateTaskStatus(name, statusCode)
